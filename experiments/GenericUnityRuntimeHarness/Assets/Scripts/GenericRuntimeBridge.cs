...
/// <summary>
/// Generic Unity Runtime Bridge — Player-side IPC handler.
///
/// Activated via [RuntimeInitializeOnLoadMethod] when UNITY_INSPECTOR_SESSION_DIR is set.
...
///
/// This is the GENERIC harness — no scene-specific logic or fixture dependencies.
/// Button.onClick listeners must be configured in the scene by the fixture project.
///
/// Commands:
...
/// </summary>
public class GenericRuntimeBridge : MonoBehaviour
{
    // ─── Env var name (neutral, fixture-agnostic) ───
    public const string EnvVarName = "UNITY_INSPECTOR_SESSION_DIR";
    ...
    /// <summary>
    /// RuntimeInitializeOnLoadMethod activator — fires BEFORE scene load.
    /// Creates the bridge GameObject automatically when the env var is present.
    /// No scene setup script required.
    /// </summary>
    [RuntimeInitializeOnLoadMethod(RuntimeInitializeLoadType.BeforeSceneLoad)]
    private static void AutoInitialize()
    {
        var sessionDir = Environment.GetEnvironmentVariable(EnvVarName);
        if (string.IsNullOrEmpty(sessionDir))
            return;

        var go = new GameObject("GenericRuntimeBridge");
        go.AddComponent<GenericRuntimeBridge>();
        DontDestroyOnLoad(go);
    }

    void Awake()
    {
        // Re-read env var in Awake for the actual instance (already verified by AutoInitialize)
    ...
    {
        Debug.Log("[GenericRuntimeBridge] Bridge started. Session dir: " + _sessionDir);
        ...
        while (true)
        {
            ...
            if (File.Exists(cmdPath))
            {
                ...
                var cmd = RuntimeJsonUtil.DeserializeCommand(json);
                if (cmd == null)
                {
    ...
    private ExecResult ProcessCommand(RuntimeCommand cmd, int index)
    {
        Debug.Log("[GenericRuntimeBridge] Executing command " + index + ": " + cmd.action);
        ...
        try
        {
            switch (cmd.action)
            {
                ...
                case "ClickButton":
                    ...
                    else
                    {
                        ...
                        else
                        {
                            btn.onClick.Invoke();
                            data["clicked"] = true;
                            Debug.Log("[GenericRuntimeBridge] Clicked button '" + goName + "'");
                        }
                    }
                    break;
                ...
                case "ObserveActiveScene":
                    ...
                    Debug.Log("[GenericRuntimeBridge] Active scene: '" + sceneName + "'");
                    break;
        ...
        catch (Exception ex)
        {
            ...
            Debug.LogError("[GenericRuntimeBridge] " + error);
        }
    ...
    private void WriteReadyMarker()
    {
        try
        {
            ...
            Debug.Log("[GenericRuntimeBridge] Ready marker written");
        }
        catch (Exception ex)
        {
            Debug.LogError("[GenericRuntimeBridge] Ready marker failed: " + ex.Message);
        }
    }
    ...
    {
        Debug.Log("[GenericRuntimeBridge] Finalizing...");
        ...
        Debug.Log("[GenericRuntimeBridge] Evidence + done written. Quitting.");
        ...
        System.Collections.Generic.Dictionary<string, object> data, string error)
    {
        try
        {
            ...
            Debug.Log("[GenericRuntimeBridge] Result " + index + " written: " + json);
        }
        catch (Exception ex)
        {
            Debug.LogError("[GenericRuntimeBridge] WriteResult failed: " + ex.Message);
        }
    }
    ...
    private static int GetParamMilliseconds(RuntimeCommand cmd, int defaultValue)
    {
        if (cmd.@params != null)
...
public static class RuntimeJsonUtil
{
    public static RuntimeCommand DeserializeCommand(string json)
    {
        try
        {
            var cmd = JsonUtility.FromJson<RuntimeCommand>(json);
